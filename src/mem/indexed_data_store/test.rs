...
mod test {
    ...
    static mut _BUFFER: [u8; 1024 * core::mem::size_of::<IndexedData<u32>>()] = [0; 1024 * core::mem::size_of::<IndexedData<u32>>()];
    static mut _BUFFER_PTR : *mut MaybeUninit<IndexedData<u32>> = unsafe{&_BUFFER[0] as *const u8 as *mut u8 as *mut MaybeUninit<IndexedData<u32>>};
    
    // static mut buf_ref : &mut MaybeUninit< &[IndexedData<Vec<u32>>]>= &mut BUF2;

    static mut _IDS : IndexedDataStore<u32> = unsafe{
    IndexedDataStore::from_raw(
     &_BUFFER_PTR, 1024, )};
    ...
    fn lifetime() {
        ...
        let _rr : &Vec<u32>;
        {
            let mut buffer2 : [MaybeUninit<IndexedData<Vec<u32>>>; 1024] = unsafe{MaybeUninit::uninit().assume_init()};
            let ptr : *mut MaybeUninit<IndexedData<Vec<u32>>> = buffer2.as_mut_ptr();


            {
                let mut ids: IndexedDataStore<Vec<u32>> = unsafe{ IndexedDataStore::from_raw( &ptr, 1024, )};
...

            {
                ...
                for k in 0..16{
                    ...
                    for i in 0..1024{
                        ...
                        assert_eq!(ids.get(&q)[0], i as u32 );
                        ids.release(q);
