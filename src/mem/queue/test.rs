...
static QUEUE: QueueUsize =
    unsafe { QueueUsize::from_static(&BUFFER[0] as *const usize as *mut AtomicUsize, 4096) };
...
fn mpmc_local() {
    unsafe {
        ...
        let queue_local =
            QueueUsize::from_static(&mut buffer_local[0] as *mut usize as *mut AtomicUsize, 4096);
...
fn mpmc_enqueue2_dequeue_wrap() {
    unsafe {
        let buffer_local: [usize; 4096] = [0; 4096];
        // unsafe { Swap::<[usize; 4096], [AtomicUsize; 4096]>::get([0; 4096]) };
        let m = QueueUsize::from_static(&buffer_local[0] as *const usize as *mut AtomicUsize, 4096);
        for _j in 0..20 {
            for i in 0..4096 {
